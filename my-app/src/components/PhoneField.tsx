import React from 'react';
import Select from 'react-select';


const PhoneField: React.FC = () => {
  const countryOptions = [
    { value: 'IN', label: 'IN +91', flag: "data:image/png;base64," },
    { value: 'US', label: 'US +00', flag: "data:image/png;base64," }
  
  ];

  const defaultCountry = countryOptions[0]; // Set India as the default country

  return (
    <div>
      <label className='text-white'>Country Code</label>
      <Select
        options={countryOptions}
        defaultValue={defaultCountry}
        formatOptionLabel={({  label, flag }) => (
          <div className='h-[20%] w-[20%]'>
            <img src={flag} alt="" className='scale-[1]' /> {label} 
          </div>
        )}
      />
     
    </div>
  );
};

export default PhoneField;
